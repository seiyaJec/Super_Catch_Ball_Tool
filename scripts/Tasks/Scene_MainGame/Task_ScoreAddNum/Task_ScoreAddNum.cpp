...
#include  "Task_ScoreAddNum.h"

namespace  ScoreAddNum
{
	//＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	// ゲームデータ
	//＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	json Data::LoadDataJson(bool useBinary_) {
		...
		if (useBinary_ == false) {
			//ファイルパス
			std::ifstream fin("./data/gamedata/scoreAddNum.json");
			if (!fin) { return json(); }
			//JSONファイル読み込み
			fin >> j;
			//ファイル読み込み終了
	...
	bool  Object::Initialize()
	{
		...
		this->scoreAdd = 0;
		this->life = this->res->data->lifeMax;
		//★タスクの生成
	...
	void  Object::UpDate()
	{
		this->pos.y -= this->res->data->moveSpeed;
	...
	{
		ge->qa_numberGUI->DrawNumber(this->pos, this->scoreAdd, true, 0, this->life / this->res->data->lifeMax);
	}
	//-------------------------------------------------------------------
	//関数定義
	//-------------------------------------------------------------------
	//スコア設定
	void Object::SetScoreAdd(int score_) {
		this->scoreAdd = score_;
	}
	//★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
	//以下は基本的に変更不要なメソッド
	//★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
