...
{
	//タスクに割り当てるグループ名と固有名
	const  string  defGroupName("数字GUI");	//グループ名
	...
	public:
		...
		//-----------------------------
		//データ変数は以下に追加する
		ML::Vec2 scorePos;			//スコアボード表示位置
		ML::Size2 num_oneOfImgSize;		//一つの画像のサイズ
		ML::Size2 num_imagesNumSize;	//各方向の分割数
		int		  num_imagesNumAll;		//分割総数
		string	  num_filePath;		//画像ファイルパス名
	...
	public:
		...
		ML::Vec2 DrawNumber(const ML::Vec2& pos_, int num_, bool useSign_ = false, int digit_ = 0, float alpha = 1.0f);		//数字を描画・左端座標を返す
	};
