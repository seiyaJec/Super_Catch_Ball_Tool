...
{
	//タスクに割り当てるグループ名と固有名
	const  string  defGroupName("スコア判定");	//グループ名
	...
	public:
		...
		ML::Vec2 displayPos;		//表示位置
		ML::Vec2 displayDistance;	//表示の間隔
		int		 digitMin;			//最小の桁数
	...
	private:
		int		score;			//現在のスコア
		int		scoreDisplay;	//表示用スコア
		int		scoreD_add;		//表示用スコア、加算値
		int		scoreD_adding;	//表示用スコア、加算中の値
	public:
		void	addScore(int adds_);		//スコア加算
		void    addScoreReflect();			//加算したスコアを反映
	};
}