...
namespace  Infomation
{
	...
	{
		string display = this->data + to_string(this->value);
		this->res->font->Draw(this->pos, display.c_str());
	}
	//-------------------------------------------------------------------
	// 関数定義
	//-------------------------------------------------------------------
	//データ設定
	void Object::SetData(const ML::Vec2& pos_, const string& data_) {
		this->pos = pos_;
		this->data = data_;
	}

	// データ設定
	void Object::SetValue(float val_) {
		this->value = val_;
	}
	//★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
	//以下は基本的に変更不要なメソッド
